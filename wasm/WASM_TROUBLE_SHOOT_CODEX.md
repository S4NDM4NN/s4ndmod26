...
### 0.21. Root-caused the dark/malformed world render, then reverted an unnecessary QVM detour

- root cause of 0.20's dark/malformed world found: `glGetFloatv(GL_PROJECTION_MATRIX)` has a side effect in gl4es — it leaves gl4es's internal "current matrix" pointer on the `PROJECTION` stack. Subsequent world-matrix uploads landed on the wrong stack (projection instead of modelview), which is what produced the dark/clipped geometry.
- fix: removed the `glGetFloatv` readbacks from the render backend's per-draw matrix audit path, added an explicit `glMatrixMode(GL_MODELVIEW)` before per-draw matrix reloads. This is a gl4es-layer fix — applies identically regardless of whether cgame/ui run as native WASM gamecode or QVM bytecode.
- separately, and *before* this root cause was known, a later session switched cgame/ui from native WASM gamecode (`WASM_NATIVE_GAMECODE=1`, `BUILD_GAME_SO=1`) to QVM bytecode (`BUILD_GAME_QVM=1`), reasoning "native gamecode conflicts with the render/pthread path" — **this reasoning is not documented anywhere else and was never verified**. In hindsight it looks like an undocumented, precautionary guess made while the real (gl4es matrix) root cause was still unknown.
- at the time of the QVM switch, native WASM gamecode had *already* had its only other blocker (the `neededDynlibs` double-load / GOT corruption crash, see `WASM_TROUBLESHOOT.md`) fixed via `MAIN_MODULE=2` → `MAIN_MODULE=1` in `iortcw/Makefile` (commit `95a1d2b6`), and had reached a connected multiplayer session with live audio (this section, above). QVM did not fix anything that wasn't already fixed on native, and introduced new regressions instead: QVM cgame crashes on connect, and QVM ui has no fonts.
- **decision: reverted to native WASM gamecode** (`WASM_NATIVE_GAMECODE=1`, `BUILD_GAME_SO=1`, `BUILD_GAME_QVM=0` in `Dockerfile`) with the gl4es matrix fix applied on top. If a future session is tempted to switch to QVM again, check first whether there's an actual reproduced crash/hang under native gamecode — as of this writing there has never been one after the `MAIN_MODULE=1` fix.
- **verification (this session)**: rebuilt and browser-tested (Playwright + headless Chromium/SwiftShader) end-to-end. Menu renders correctly with fonts. `connect 127.0.0.1` to the local dedicated server succeeds — no `invoke_vi`/GOT-corruption crash, cgame loads, gameplay runs (bots fight, kill feed updates). This confirms native gamecode is solid and strictly ahead of where QVM ever got.
- **also found and removed**: `RB_RenderDrawSurfList()` in `tr_backend.c` had a leftover `#ifdef __EMSCRIPTEN__` debug block that unconditionally `return`ed before drawing any surfaces for non-`RDF_NOWORLDMODEL` scenes (i.e. it silently no-op'd every in-game world draw call, logging `"WASM skipping 3D world draw"` for the first 8 frames only). This was almost certainly left in from the darkness-diagnosis session and explains why the world appeared black/empty even after other fixes landed — it was never actually attempting to draw. Removed it.
- **still broken after removing the skip**: with the skip gone, `numDrawSurfs` correctly reaches 1000+ per frame and `RB_RenderDrawSurfList` now actually runs its draw loop, but the canvas is still **fully black at the pixel level** — verified directly via `gl.readPixels()` from within the page (not just a screenshot), 0% non-black pixels across the whole 1024x768 frame. So the documented gl4es `glGetFloatv` matrix fix by itself does **not** fully resolve the dark world — there is at least one more bug here. Also newly visible now that surfaces are actually drawing: repeated `WebGL: INVALID_ENUM: readPixels: invalid format` warnings (likely from the `r_measureOverdraw` stencil readback path or `tr_flares.c`'s depth readback under gl4es/WebGL) and `WebGL: INVALID_OPERATION: texParameter: no texture bound to target` during map load — either could be a lead, neither confirmed yet.
- **status for next session**: native gamecode is the right foundation (confirmed working up through connect/gameplay-logic); the world-render darkness needs a fresh diagnosis pass — do not assume the old gl4es matrix writeup already covers it, since removing the debug skip shows it isn't fully fixed.

### 0.22. ACTUAL root cause found and fixed: `SDL_QuitSubSystem(SDL_INIT_VIDEO)` on vid_restart permanently breaks WASM canvas presentation

Everything in 0.20/0.21 was a red herring. The gl4es matrix state, texture bindings, vertex/color arrays, and shader stages were all *already correct* — proven by adding diagnostics that log the actual GL state (matrix mode, bound texture vs. expected texnum, computed NDC vertex coordinates, `glGetError()`) right at the draw call in `RB_WasmDirectStretchPic` / `R_DrawElements`. Every value checked out. The world, HUD, and console were being drawn correctly the entire time — the browser just never displayed any of it.

**How this was found**: took repeated screenshots spaced several seconds apart during active gameplay (bots fighting, kill feed updating) and diffed them byte-for-byte. They were identical. Meanwhile the renderer's own diagnostics kept firing draw calls with fresh, correct data. That combination — internal state changing, visible output frozen — means the bug is in frame *presentation*, not drawing. Narrowed the exact moment with per-second, then per-200ms, screenshots through the connect sequence: the display freezes on the "Awaiting gamestate..." connecting screen almost immediately after issuing `connect`, well before cgame or the world are even touched.

**Root cause**: connecting to a server whose game directory differs from what's currently loaded (`main` → `s4ndmod26` mod, which is *every* connect from a fresh launch) triggers a `Client Shutdown (Game directory changed)` → full `vid_restart`-equivalent (`RE_Shutdown` + `R_Init`). `GLimp_Shutdown()` in `iortcw/code/sdl/sdl_glimp.c` called `SDL_QuitSubSystem(SDL_INIT_VIDEO)` unconditionally as part of that cycle. Tearing down and reinitializing SDL's entire video subsystem is normal and safe on native platforms, but under Emscripten it permanently orphans the browser's canvas presentation — the browser keeps compositing whatever frame was on screen at the moment of shutdown, forever, even though `GLimp_SetMode()` successfully creates a brand new working WebGL context afterward and the engine renders correctly into it from then on.

**Fix** (both in `iortcw/code/sdl/sdl_glimp.c`):
1. `GLimp_Shutdown()`: skip `SDL_QuitSubSystem(SDL_INIT_VIDEO)` entirely under `__EMSCRIPTEN__` — there is only ever one canvas for the lifetime of the page, so there's nothing to legitimately tear down mid-session.
2. `GLimp_SetMode()`: as defense in depth, if a window/GL context already exist when re-entering this function (which can still happen via other callers), reuse them instead of destroying and recreating — skip straight to `GLimp_DetectAvailableModes()` and return, rather than going through the `SDL_DestroyWindow`/`SDL_CreateWindow`/`SDL_GL_CreateContext` cycle.

**Verified end-to-end**: menu renders, connect succeeds, the actual `mp_beach` map renders correctly (sky, terrain, fences, obstacles, particles), bots visibly move frame to frame, kill-feed text updates, and the console now renders at the correct half-screen size with legible text (this was the same "tiny collapsed" symptom as the world — same root cause, same fix). Confirmed via 5 screenshots taken 3s apart during gameplay all being distinct (previously identical).

**Lesson for future sessions**: when a symptom looks like "everything downstream of X is subtly wrong" (matrix state, texture binding, blend state, etc.) but every individual piece of state checks out correct when actually inspected, suspect the *display/presentation* layer before the *rendering* layer. A `page.screenshot()`-style diff over time (is the visible output changing at all?) is a cheap, decisive test that should happen early, not after days of GL-state archaeology.

### 0.23. Second major root cause: cgame/ui side modules export identically-named globals, colliding across the shared GOT

With 0.22 fixed, new symptoms surfaced now that the display actually updates: `cg_animspeed` and `cg_draw2D` behaved as if permanently 0 (frozen player/bot animations, no 2D HUD), console keyboard input sometimes fell through to chat instead of executing as a command, and the limbo/class-selection menu wouldn't open.

**Root cause**: `mod/src/cgame/cg_main.c` and `mod/src/ui/ui_main.c` both declare a global `cvarTable_t cvarTable[]` (and `int cvarTableSize`) — plain globals, not `static`. Under Emscripten's native dynamic linking, `cgame.mp.wasm` and `ui.mp.wasm` are both `SIDE_MODULE`s sharing one GOT (global offset table) for cross-module symbol resolution. When two side modules export a symbol with the *same name*, only the first-loaded module's copy actually gets used — per the mechanism already documented in `WASM_TROUBLESHOOT.md` for the `neededDynlibs` double-load bug (`updateGOT`'s `replace=false` skips already-set entries). Since `ui.mp.wasm` loads before `cgame.mp.wasm`, cgame's own references to its `cvarTable` were silently redirected into UI's `cvarTable` memory — cgame's `CG_RegisterCvars()` was iterating over *UI's* cvar list. Confirmed by adding a print inside the registration loop: it logged `ui_glCustom`, `ui_ffa_fraglimit`, etc. (102 entries, UI's table size) instead of cgame's own ~180 entries. `cg_animspeed`/`cg_draw2D` (and everything past UI's shorter table length) never got registered with the engine at all — confirmed via console: `/cg_animspeed` returned `unknown cmd cg_animspeed` while `/cg_viewsize` (registered earlier, within the overlapping range) printed correctly.

**Fix, part 1**: marked `cvarTable`/`cvarTableSize` `static` in both `cg_main.c` and `ui_main.c` (safe — nothing outside those files referenced them).

**Fix, part 2 — the general case**: the same collision risk exists for *any* identically-named global between the two modules, most dangerously the ~100 `trap_*` syscall stub functions in `cg_syscalls.c`/`ui_syscalls.c` (both define `trap_R_RenderScene`, `trap_Cvar_Set`, etc. under the same names, each calling a different numeric syscall enum). **Do not fix this by adding `static` to the trap_* functions individually** — unlike `cvarTable`, these are called from dozens of files *within* each module, so `static` (file-scope-only linkage) breaks the build immediately (confirmed: native Linux/Windows builds fail with "static declaration follows non-static declaration" against the `extern`-by-default header prototypes in `cg_local.h`/`ui_local.h`). The correct fix is a compiler visibility flag: added `-fvisibility=hidden` to the Emscripten `SHLIBCFLAGS` in `iortcw/Makefile` (native Linux builds already do this — emscripten's was empty). This hides every symbol by default (fixing all *current and future* same-name collisions in one place) while `dllEntry` and `vmMain` — which must stay visible for the engine's `Sys_LoadGameDll()` → `dlsym(handle, "vmMain")` lookup — are unaffected because they're already marked `Q_EXPORT` (`__attribute__((visibility("default")))`).

**Verified**: rebuilt, `cg_animspeed`/`cg_draw2D`/`cg_skybox`/`cg_viewsize` all resolve correctly via console now; the limbo/class-selection menu (chat box, objectives tabs, team select buttons) rendered correctly and appeared spontaneously during a routine test, confirming UI-side logic that depends on trap calls / cvars now works.

**Lesson for future sessions**: in a WASM build using native dynamic linking between multiple side modules (not QVM), *any* non-`static` global — function or data — with the same name in two different modules is a silent collision risk. Prefer `-fvisibility=hidden` (+ explicit `Q_EXPORT` on the handful of symbols that must cross the module boundary) over auditing/fixing individual symbols one at a time.
